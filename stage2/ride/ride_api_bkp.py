...
def add_ride():
    ...
    #      environ_base={'REMOTE_ADDR': userMicroService})
    para1 = json.dumps(para1)
    print("PARA1 JSON---------", para1)
    para1 = json.loads(para1)

    # resp1 = requests.post('http://171.19.0.2:5000/api/v1/db/read',\
    #     json=para1_jobj,follow_redirects=True)
    print("*****************")

    resp1 = requests.post("http://localhost:5000/api/v1/db/read",\
        json=para1)

    resp1 = ast.literal_eval(resp1.text)
    print("*****************", resp1)

    # localhost:8000/api/v1/rides 
